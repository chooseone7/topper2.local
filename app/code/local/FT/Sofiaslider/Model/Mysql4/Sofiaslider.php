<?php
class FT_Sofiaslider_Model_Mysql4_Sofiaslider extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct()
    {    
        // Note that the slide_id refers to the key field in your database table.
        $this->_init('sofiaslider/slides', 'slide_id');
    }
	...
	protected function _afterSave(Mage_Core_Model_Abstract $object)
	{
		...
		$this->_getWriteAdapter()->delete($this->getTable('sofiaslider/slides_store'), $condition);
		...
		foreach ((array)$object->getData('stores') as $store) {
			...
			$this->_getWriteAdapter()->insert($this->getTable('sofiaslider/slides_store'), $storeArray);
		}
	...
	protected function _afterLoad(Mage_Core_Model_Abstract $object)
	{
		...
		$select = $this->_getReadAdapter()->select()
			->from($this->getTable('sofiaslider/slides_store'))
